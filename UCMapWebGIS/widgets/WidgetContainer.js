    ...
    function (declare, win,array, query, topic, domStyle, on, coreFx, fx, lang, domAttr, domClass, domGeom, nodeListFx, ContainedWidgetFrame, MoveableWidgetFrame, _Widget, _Container, Moveable, _TemplatedMixin, template) {
        return declare([_Widget, _TemplatedMixin, _Container], {
            ...
            postCreate: function () {
                ...
                topic.subscribe("showMapResources", lang.hitch(this, "onShowMapResources"));
            },
            ...
            onShowWidget: function (widget) {
                ...
                if (widget) {
                    ...
                    if (!bFound) {
                        ...
                        if (widget.declaredClass === 'dijit._WidgetsInTemplateMixin') {
                            var frame = new ContainedWidgetFrame();
                            frame.setWidget(widget);
                        }
                        //console.log(widget.declaredClass === 'dijit._WidgetsInTemplateMixin');
                       // console.log(widget.declaredClass);
                       // console.log(widget.isInstanceOf(_Widget));
            ...
            //onShowMapResources创建了一个可移动框架实例，改实例的打开，关闭，最小化均由可移动框架控制，
            onShowMapResources: function (widget) {
                //获取地图图层的dom节点，
                var tarNode = document.getElementById("mapDiv");
                //根据可移动框架的id获取其dom节点
                var mapResLoaded = document.getElementById("dragHandle");
                //创建MoveableWidgetFrame实例前，可移动框架的dom树没有加载到tarNode，此时mapResLoaded是false
                //创建MoveableWidgetFrame实例后，mapResLoaded是true,这样的设置可以防止多次创建可移动框架实例，只有在可移动框架关闭并移除其dom树后才可以进入下面的循环
                //_BaseWidget是_Widget的一个实例，_BaseWidget是可移动框架的内容框
                if (widget.isInstanceOf(_Widget) && !mapResLoaded) {
                    var frame = new MoveableWidgetFrame();
                    frame.setWidget(widget);
                    domStyle.set(frame.domNode, "top", "40%");
                    domStyle.set(frame.domNode, "left", "40%");                
                    frame.placeAt(tarNode);
                    frame.startup();
                };
