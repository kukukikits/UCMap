        ...
        return declare([_BaseWidget, _WidgetsInTemplateMixin], {
            ...
            postCreate: function () {
                try {
                    ...
                    buttons.forEach(function (item, idx, arr) {
                        var icon = require.toUrl("UCMapWebGIS/widgets/" + domAttr.get(item, "buttonIcon"));
            ...
            startup: function () {
                ...
                try {
                   ...
                   // console.log(this.widgets.colorDropDown.dropDown);
                    var item = this.widgets.colorDropDown.dropDown;
                   // console.log(this.widgets.colorDropDown);
                    ...
                    //alert(this.widgets.colorPalette);
            ...
            addGeometry: function (geometry) {
                if (geometry) {
                    try {
                        // 处理加入文本
                       // alert(this._lastDrawToolText);
                        if (geometry.declaredClass === "esri.geometry.Point" && this._lastDrawToolText === "增加文本") {//这一句没进来
            ...
            clearGraphics: function () {
                if (this.map) {
                    try {
                       ...
                       // alert(this.map.graphics.graphics.length - 1);
                        for (var i = this.map.graphics.graphics.length - 1; i >= 0; i--) {
                            var g = this.map.graphics.graphics[3];
                            this.map.graphics.clear(g);
                           // console.log(i);
            ...
            stepback: function () {
                if (this.map) {
                    try {
                        //--下面的语句可以一个一个地删除图形----------！  
                        var i = this.map.graphics.graphics.length - 1;
                        this.count = i;
                        var g = this.map.graphics.graphics[this.count];
                        if (this.isMyGraphic(g) && this.count != 0) {
                            this.map.graphics.remove(g);
                            this.count--;
                        }
                    }
