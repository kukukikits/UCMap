define([
        ...
        query)
    {
        return declare(DynamicMapServiceLayer, {
                        ...
                        if (parsedData.data.hasOwnProperty(xParsed)) {
                            ...
                            for (yParsed in parsedData.data[xParsed]) {
                                if (parsedData.data[xParsed].hasOwnProperty(yParsed)) {
                                    ...
                                    // console.log(parsedData.data[xParsed][yParsed].count);
                                }
                            }
                        }
                    }
            ...
            parseHeatmapData: function (Points) {
                // variables
                ...
                if (Points) {
                    ...
                    for (i = 0; i < Points.length; i++) {
                        ...
                        dataPoint = Point(Points[i].feature.geometry);
                        // check point
                        ...
                        if (validPoint) {
                            // attributes
                            attributes = Points[i].feature.attributes;
                            ...
                            if (!parsedData.data[dataPoint.x][dataPoint.y]) {
                                ...
                                if (attributes && attributes.hasOwnProperty(this.properties.tempDataField)) {
                                    // create array value with count of count set in datapoint
                                    parsedData.data[dataPoint.x][dataPoint.y].count = attributes[this.properties.tempDataField]; 
            ...
            // 设置数据
            setData: function (idResults) {
                // set width/height
                this.resizeHeatmap(null, this._map.width, this._map.height);
                this.lastData = idResults;
               this.parseHeatmapData(idResults);
                // 重新绘制热度图
                this.refresh();
